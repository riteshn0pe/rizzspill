...
class ChatBloc extends HydratedBloc<ChatEvent, ChatState> {
  ...
  void _onStartAiSession(StartAiSession event, Emitter<ChatState> emit) {
    print("[BLOC] Starting Session: ${event.roomId}");
    
    // 1. CHECK FOR NEW SESSION
    if (_currentRoomId != event.roomId) {
      print("[BLOC] New Room Detected.");

      // --- RESCUE LOGIC: Sync old data before wiping ---
      if (!_isSynced && _aiMessages.isNotEmpty) {
        print("[BLOC] ⚠️ Found unsynced data. Attempting Rescue Sync...");
        
        // Create a snapshot (COPY) of the old data
      ...
      // NOW it is safe to reset for the new user
      _aiMessages = [];
      ...
      print("[BLOC] Resuming Existing Session.");
      // Same Session -> Resume
      if (!_isSynced && _aiMessages.isNotEmpty) {
